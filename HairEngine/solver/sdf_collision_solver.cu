...
void SDFCollisionSolver_cudaResolveCollisionKernal(float3 *parPoses, float3 *parVels, const unsigned char *parLocalIndices, const float3 * vels,
                                                   const unsigned long long *grid, int npar, float3 origin, float3 d, int3 n, float contour, float fraction, bool changeHairRoot) {
...
void SDFCollisionSolver_cudaResolveCollision(float3 *parPoses, float3 *parVels, const unsigned char *parLocalIndices, const float3 * vels,
	...
	SDFCollisionSolver_cudaResolveCollisionKernal<<<nblock, nthread>>>(parPoses, parVels, parLocalIndices, vels, grid, npar, origin, d, n, contour, fraction, changeHairRoot);
	cudaDeviceSynchronize();
}
