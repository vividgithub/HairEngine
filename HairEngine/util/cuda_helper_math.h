...
inline __device__ __host__ uint32_t floatflip(float a) {
	uint32_t aint = *((uint32_t*)(&a));
	return (aint << 1) | (aint >> 31);
}

inline __device__ __host__ float ifloatflip(uint32_t a) {
	a = (a >> 1) | (a << 31);
	return *((float*)(&a));
}

inline __device__ __host__ float cudaPointToLineSegmentDistance(float3 p, float3 x0, float3 x1) {
	float3 x10 = x1 - x0;
	float l = length(x10);

	float t = dot(x1 - p, x10) / (l * l);
	t = fmaxf(0.0f, fminf(t, 1.0f));

	return length(p - (t * x0 + (1.0f - t)*x1));
}

inline __device__ __host__ float cudaPointToTriangleSignedDistance(float3 p, float3 x0, float3 x1, float3 x2) {
	...
	float3 nTri = cross(x2 - x0, x1 - x0);

	const float tol = 1e-8f;

	if (a >= -tol && b >= -tol && c >= -tol) {
		// Inside the triangle
		d = length(p - (a * x0 + b * x1 + c * x2));
	}
	else
	{
		d = fminf(cudaPointToLineSegmentDistance(p, x0, x1),
		          cudaPointToLineSegmentDistance(p, x1, x2));
		d = fminf(d, cudaPointToLineSegmentDistance(p, x0, x2));
	}
